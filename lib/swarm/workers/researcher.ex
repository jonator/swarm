defmodule Swarm.Workers.Researcher do
  ...
  def perform(%Oban.Job{args: %{"agent_id" => agent_id}}) do
    ...
    with %Agent{} = agent <- Agents.get_agent(agent_id),
         {:ok, agent} <- Agents.mark_agent_started(agent),
         {:ok, result} <- conduct_research(agent),
         {:ok, agent} <- Agents.mark_agent_completed(agent) do
      ...
      {:error, reason} = error ->
        ...
        error
      nil ->
        error = {:error, "Agent not found: #{agent_id}"}
        Logger.error("Research agent #{agent_id} failed: Agent not found")
        error
    end
  end
  ...
  defp count_files_in_index(index) do
    # This would depend on the Git.Index implementation
    # For now, return a placeholder
    0
  end
  
  defp find_key_files(index) do
